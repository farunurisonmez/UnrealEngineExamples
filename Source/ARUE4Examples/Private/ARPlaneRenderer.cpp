...
void AARPlaneRenderer::DrawPlanes()
{
...
void AARPlaneRenderer::UpdatePlane(UARPlaneGeometry* ARCorePlaneObject)
{
	...
	if (ARCorePlaneObject->GetTrackingState() == EARTrackingState::Tracking && ARCorePlaneObject->GetSubsumedBy() == nullptr)
	{
		if (PlanePolygonMeshComponent->bHiddenInGame)
		{
			PlanePolygonMeshComponent->SetHiddenInGame(true, true);
		}
		UpdatePlaneMesh(ARCorePlaneObject, PlanePolygonMeshComponent);
	}
	else if (PlanePolygonMeshComponent->bHiddenInGame)
	{
		PlanePolygonMeshComponent->SetHiddenInGame(false, true);
	}
	if (ARCorePlaneObject->GetSubsumedBy() != nullptr || ARCorePlaneObject->GetTrackingState() == EARTrackingState::StoppedTracking)
	{
		PlanePolygonMeshComponent = *PlaneMeshMap.Find(ARCorePlaneObject);
